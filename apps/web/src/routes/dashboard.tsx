  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Stat label="Enviados no período" value={data?.sent ?? 0} />
        <Stat label="Pendentes na fila" value={data?.pending ?? 0} />
        <Stat label="Grupos ativos" value={data?.groups ?? 0} />
        <Stat label="Falhas no período" value={data?.failed ?? 0} />
      </div>
